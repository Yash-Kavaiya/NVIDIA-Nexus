...
class TaskService:
    async def get_tasks(
        ...
        """Get all tasks with their steps"""
    ...
    async def update_task_progress(
        self, db: AsyncSession, task_id: str, progress: float
    ) -> Optional[Task]:
    ...
    async def complete_task_step(
        self, db: AsyncSession, step_id: str, result: str = ""
    ) -> Optional[TaskStep]:
    ...
    async def _recalculate_task_progress(self, db: AsyncSession, task_id: str):
        ...
        result = await db.execute(select(TaskStep).where(TaskStep.task_id == task_id))
    ...
    async def create_task(
        self, db: AsyncSession, title: str, description: str, steps: list[dict] = None
    ) -> Task:
