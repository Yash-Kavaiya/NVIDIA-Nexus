...
resource "aws_efs_file_system" "uploads" {
  ...
  tags = merge(local.common_tags, { Name = "${var.project_name}-uploads" })
}
...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "DATABASE_URL", value = "sqlite+aiosqlite:///./data/nvidia_nexus.db" },
      { name = "DEBUG", value = "False" }
    ]

    secrets = [{
      name      = "NVIDIA_API_KEY"
      valueFrom = aws_secretsmanager_secret.nvidia_api_key.arn
    }]
